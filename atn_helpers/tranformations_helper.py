...
def transfromation_parameters_regressor(transformations,images,keep_prob,input_sz,batch_size,num_channels=1,
    ...
    num_of_params = 7
    if transformations[0] != "ane":  #since the exponent should be taken, we'll learn the parameters up front. when ane appears, it is expected to be the only transformation requested, so this is valid.
        #        params = regress_parameters_conv(images,width,keep_prob,activation,num_of_params, weight_stddev, num_channels)
        params = regress_parameters(images,input_sz,keep_prob,activation,num_of_params,weight_stddev,num_channels)
    else:
        params = learn_all_params_conv(images,input_sz,keep_prob,activation,weight_stddev,num_channels)
    ...
    for transformation in transformations:
        if transformation in methods:
            ...
            transformation_exponentiol,orig_transformation, d2 = translation(params,batch_size,num_channels)

            orig_transformation = tf.slice(params,[0,0],[-1,6])  # !!!! OVERWRITE THE THETA TO BE FULL AFFINE !!!
    ...
    if len(transform_maps) > 1:
        #return tf.reshape(transformations_composition(transform_maps),[-1,6]),transform_maps_dict, d2
        return orig_transformation, transform_maps_dict,d2
    else:
        transformation = tf.slice(transform_maps[0],[0,0,0],[-1,2,3])
        #return tf.reshape(transformation,[-1,6]),transform_maps_dict, d2
        return orig_transformation, transform_maps_dict, d2


#this is the stn's localization network
#here we construct it using a 2-layer FC network.
def regress_parameters(x,input_sz,keep_prob,activation_func,num_of_params,weight_stddev,num_channels):
    with tf.variable_scope("atn"):
        #We'll setup the two-layer localisation network to figure out the
        # parameters for an affine transformation of the input
        # Create variables for fully connected layer
        W_fc_loc1 = weight_variable([input_sz * input_sz * num_channels,20],weight_stddev)
